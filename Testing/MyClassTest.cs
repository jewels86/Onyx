    ...
    {
        {
            ...
            Console.WriteLine("MyClass has private fields:");
            foreach (var field in privateFields)
            { 
                Console.WriteLine($"Name: {field.Name}, Type: {field.Type}, Value: {field.Value}, Access: {field.Access}"); 
                Reflection.SetField(myClass, field.Name, null!); // Lets mess it up
            }
                    
            // Now lets try to use MyClass
            Console.WriteLine("Now trying to use MyClass:");
            myClass.PrintX();
        ...
        }
        {
            ...
            // We can do this by creating a new type with an implicit int operator
            ...
            var (evilIntType, tctx) = dtb.Build(); // we also could've used a base class and changed the ToString() if we wanted to specifically activate on PrintX
